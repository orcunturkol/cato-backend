...
public class UpdateActionValidator : AbstractValidator<UpdateActionCommand>
{
    ...
    public UpdateActionValidator()
    {
        ...
        RuleFor(x => x.ActionType).Must(t => ValidActionTypes.Contains(t!))
            .When(x => x.ActionType is not null)
        ...
        RuleFor(x => x.ActualCostUsd).GreaterThanOrEqualTo(0).When(x => x.ActualCostUsd.HasValue);
    }
}
