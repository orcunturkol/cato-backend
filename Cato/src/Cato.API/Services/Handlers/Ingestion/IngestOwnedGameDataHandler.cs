...
public class IngestOwnedGameDataHandler : IRequestHandler<IngestOwnedGameDataCommand, IngestionResult>
{
    ...
    private readonly ILogger<IngestOwnedGameDataHandler> _logger;

    public IngestOwnedGameDataHandler(CatoDbContext db, ILogger<IngestOwnedGameDataHandler> logger)
    {
        _db = db;
        _logger = logger;
    }

    public async Task<IngestionResult> Handle(IngestOwnedGameDataCommand request, CancellationToken ct)
    {
