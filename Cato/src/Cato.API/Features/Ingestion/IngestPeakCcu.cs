...
public static class IngestPeakCcu
{
    ...
    public class Endpoint : ICarterModule
    {
        ...
        {
            app.MapPost("/api/ingestion/peak-ccu", async (Command command, IMediator mediator) =>
            {
                var result = await mediator.Send(command);
                return Results.Ok(result);
            })
            .WithTags("Ingestion")
            .WithDescription("Ingest peak CCU history from JSON file");
        }
    }
}
